...
colnames(team_stats) <<- c('teamID', 'Runs', 'Wins')
...
# use the sum of the input values for teams to predict the total team output value 
  ...
  output$summary <- renderPrint({summary(createModel(input$predictor))})
  output$anova <- renderPrint({anova(createModel(input$predictor))})
})
